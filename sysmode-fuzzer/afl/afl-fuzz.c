...
static char* checkpoint_dir;          /* Directory for CRIU checkpoint    */
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  // Once we have the PID, we can start a timer to execute the child
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Qc:p:")) > 0)
    ...
    switch (opt) {
      ...
      case 'p': /* path to CRIU checkpoint dir */
  ...
  if (count != -1) {
      ...
      fprintf(stderr, "Failed to find directory of afl-fuzz!\n");
      exit(5);
  }

  char path_to_fuzzer[PATH_MAX];
  path_to_fuzzer[0] = 0;
  strcat(path_to_fuzzer, sync_dir);
  strcat(path_to_fuzzer, "/");
  strcat(path_to_fuzzer, sync_id);
  strcat(path_to_fuzzer, "/");

  target_path = strcat(path,"/sm-fuzzer");
  use_argv[0] = path;
  use_argv[1] = path_to_fuzzer;
  use_argv[2] = checkpoint_dir;
  use_argv[3] = 0;
